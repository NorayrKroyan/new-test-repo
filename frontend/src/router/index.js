...
router.beforeEach(async (to) => {
    ...
    if (to.meta.publicGuest) {
        try {
            ...
            return customerSession?.user?.must_change_password ? '/customer/change-password' : '/customer/dashboard'
        } catch {}

        try {
            const carrierSession = await fetchCarrierMe()
            return carrierSession?.user?.must_change_password ? '/carrier/change-password' : '/carrier/dashboard'
        } catch {}
